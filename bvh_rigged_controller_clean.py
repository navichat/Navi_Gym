...
This controller creates a simple but effective BVH-driven animation system
that moves the entire avatar as a coherent unit while applying BVH motion data
for realistic walking and movement patterns.
"""
...
class BVHPuppetController:
        ...
        for name, entity in avatar_entities:
            if entity:
                try:
                    ...
                    self.original_positions[name] = np.array(pos)
                    self.original_rotations[name] = np.array(rot)
                    log_status(f"  📍 Stored original transform for {name}")
                except:
                    # Fallback if getting transforms fails
        ...
        for name, entity in self.avatar_entities:
            if entity and name in self.original_positions:
                try:
                    ...
                    # Add subtle random variation for more lifelike movement
                    ...
                    # For fixed entities, this may not work, but we try anyway
                    ...
                    # For fixed entities, updates may fail - that's expected
                    pass
    ...
    def update_animation(self, delta_time: float):
        ...
        # Apply to avatar (this may not work for fixed entities, but we try)
